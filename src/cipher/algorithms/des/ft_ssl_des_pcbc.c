...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/22 12:22:15 by pheilbro          #+#    #+#             */
/*   Updated: 2019/10/24 10:49:50 by pheilbro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_ssl_des_pcbc_encode_block(t_des_context *c)
{
	int			i;
	uint64_t	plaintext;
	...
	c->init_vector = c->block ^ plaintext;
	ft_dstr_add(c->out, (char *)(c->block), 16);
}

void	ft_ssl_des_pcbc_decode_block(t_des_context *c)
{
	int			i;
	uint64_t	ciphertext;
	...
	scramble_des_block(&(c->block), g_init_permutation_tab);
	c->block ^= c->init_vector;
	c->init_vector = c->block ^ ciphertext;
	ft_dstr_add(c->out, (char *)(c->block), 16);
}
